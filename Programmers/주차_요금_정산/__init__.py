...
def solution(fees, records):
    [standardTime, standardFee, perTime, perFee] = fees
    cars = {}

    for record in records:
        carNumber = record.split(" ")[1]
        if not carNumber in cars:
    ...
    for record in records:
        ...
        if status == "IN":
            ...
            cars[carNumber]["accTime"] += calculateAccumulateTime(
                cars[carNumber]["time"], time
            )
            cars[carNumber]["status"] = status
    ...
    for carNumber in cars:
        if cars[carNumber]["accTime"] <= standardTime:
            ...
            cars[carNumber]["totalFee"] = (
                standardFee
                + ceil((cars[carNumber]["accTime"] - standardTime) / perTime) * perFee
            )

    result = []
    for carNumber in cars:
        result.append([carNumber, cars[carNumber]["totalFee"]])
    result.sort(key=lambda item: int(item[0]))

    return [totalFee for _, totalFee in result]


def calculateAccumulateTime(start, end):
